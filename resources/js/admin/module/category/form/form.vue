...
export default {
    ...
    methods: {
        ...
        getCategory(formValue) {
            this.$options.service.getCategory(response => {
                formValue.getFields().forEach(item => {
                    item.field.fieldObject.value = response.data.category[item.field.fieldObject.name];
                });
                this.formValue = formValue;
            });
        },
        ...
        back() {
            this.$router.push({name: 'category.list', params: {typeId: this.$options.service.typeId}});
        }
    },
    components: {
        pageBox,
